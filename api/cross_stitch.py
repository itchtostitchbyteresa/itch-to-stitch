...
DMC: List[Dict] = [
    ...
    {"code": "3863", "r": 164, "g": 131, "b": 92, "name": "Mocha Beige Med"},
...
def _measure(drw: ImageDraw.ImageDraw, text: str, font: ImageFont.ImageFont) -> Tuple[int,int]:
    # Pillow ≥10: use textbbox; fallback to font.getbbox when needed
    try:
...
def render_chart(quantized: Image.Image, legend: List[Dict]) -> Image.Image:
    ...
    # symbols (use textbbox instead of textsize)
    for y in range(h):
        for x in range(w):
            rgb = qpx[x, y]
...
async def cross_stitch(
    ...
    try:
        ...
        qimg, legend = (_quantize_numpy(img) if HAS_NUMPY else _quantize_pure(img))
